...
/*
    Class du serceur de jeux  dominos

    Events:
        begin : debut de la partie
        end: fin de  partie
*/
...
Domino.Server.prototype._emitGameover = function emitGameover() {
    ...
    this.Events.emit('end', turnInfos);//Event de fin de partie
};
    ...
    this._io.on('connection', function (socket) {
        ...
        socket.on('game', function (data) {
            ...
            if (that._socketsClient.item(socket.id)) {
                ...
                that.Events.emit('init', { id: socket.id });
            }
        });
        ...
        socket.on('start', function () {
            ...
            if (that._socketGame != undefined) {
                if (that._game.isGameLaunched === false) {
                    ...
                    that._kickAutoplay();
                    that.Events.emit('begin', turnInfos);
                }
                else {
