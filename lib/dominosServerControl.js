...
Domino.Server = function Server(httpServer, garbageDelay) {
    this.debug = Debug('domino.server');
    
    this._game = new Game();
    
    this._io = socketIO.listen(httpServer);
    ...
    this._io.sockets.on('connection', function (socket) {
        ...
        socket.on('player', function (data) {
            ...
            if (that._socketGame != undefined) {
                ...
                that._io.sockets.emit('new_player', result);                
            }
            else {
        ...
        socket.on('start', function () {
            ...
            if (that._socketGame != undefined) {
                if (that._game.isGameLaunched === false) {
                    ...
                    that._io.sockets.emit('start', turnInfos);
                }
                else {
        ...
        socket.on('play', function (data) {
            ...
            if (client) {
                ...
                if (that._socketGame != undefined) {
                    ...
                    that._socketsClient.item(data.id).emit('play', turnInfos);
                    
                    if (turnInfos.ending) {
                        turnInfos = that._game.getWinner();
                        that.debug(' fin du jeux - %s', JSON.stringify(turnInfos));
                        /* ** Message TOUS ** *"end"* Fin de la partie avec le gagnant ! "*/
                        that._io.sockets.emit('end', turnInfos);
                    }
                    else {                        
                        ...
                        that._io.sockets.emit('turn', turnInfos);
                    }
                }
                else {
                    that._io.sockets.emit('error', { err: 1, msg: 'Jeu déconnecté !' });
                }
            }
        });
        ...
        socket.on('pick', function (data) {
            ...
            if (client) {
                ...
                if (that._socketGame != undefined) {
                    ...
                    that._socketsClient.item(data.id).emit('pick', turnInfos);
                ...
                if (that._socketGame.id == socket.id) {
                    ...
                    that._socketsClient.forEach(function (remote) {
                        if (remote.type == VirtualJoystick.Type.remote) {
                            remote.send('close', { id : that._socketGame.id });
                        }
                    });
