...
/******************************************************************************/
/*                Option : table for content if defined locally               */
/******************************************************************************/

/*
          var filename = "./NFCdata.json";

          var NFCTable;
          fs.readFile(filename, 'utf8', function (err, data) {
            if (err) throw err;
            NFCTable = JSON.parse(data);
          });
    */
...
/*
pollNFC = function pollNFC(delay) {
  setInterval(pushNFC, delay);
};
*/
//90A2DAF80880
...
var hostsAddress = '172.21.254.58';//Version MediaLab
...
Broadsign.pushNFC = function pushNFC() {
    ...
    try {
        ...
        client.on('error', function (err) {
            debug('Connect ERROR to Broadsign player (pushNFC)', err);
...
Broadsign.pushTrigger = function pushTrigger(id) {
    try {
        ...
        client.on('error', function (err) {
            debug('Error to write trigger Broadsign player (pushTrigger)', err);
        });
        client.on('close', function (had_error) {
...
var _loopNFCAction = function _loopNFCAction() {
    ...
    if (loopNfcContentIndex < loopNfcContent.length) {
        ...
        try {
            ...
            client.on('error', function (err) {
                debug('Connect ERROR to Broadsign player (_loopNFCAction)', err);
