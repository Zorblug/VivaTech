    ...
    if (!_that._cam) {
      ...
      _that._cam.open()
        ...
        .then(function (cam) {
          ...
          // debug('Camera port open :' + JSON.stringify(cam._com));
          return _that._cam.setCameraAngle(3);
        })
        .then(function (answer) {
          if (answer.isError) {
            debug('Answer ERROR :', answer);
          }
          return _that._cam.getCameraAngle();
        })
        .then(function (answer) {
          if (answer.isError) {
            debug('Answer ERROR :', answer);
            ...
            debug('Answer camera angle : ' + answer.degre());
          }
          return _that._cam.setFaceAngles (1, 0);
        })
        .then(function (answer) {
          if (answer.isError) {
            debug('Answer ERROR :', answer);
  ...
  var p = new Promise(function (resolve, reject) {
    if (_that._cam) {
      if (!_that._cam._loopRun) {
        _that._cam.startLoop(_that._options, callback, 1500);
      }
      resolve(_that);
    }
    else {
