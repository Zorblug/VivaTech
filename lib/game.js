 ...
 *  La table de jeux est un tableau de domino ou chaque domino est rangé et tourné de façon à représenter les dominos posés: [1,1],[1,4],[4,5],[5,3] etc..
...
Game.prototype.addNewPlayer = function addNewPlayer(id, playerData) {    
    if (this.isGameLaunched === false) {
        if (this._players.count < this.maxCount) {
            ...
            this.debug("Too many players!!");
            return { result: -1, count: this._players.count, player:undefined };
        }
    }
    else {
        this.debug("La partie est lancée !!");
...
Game.prototype.getActivePlayer = function getActivePlayer() { 
    return this._players.item(this._playersOrder[this.turn]);
};
...
Game.prototype.currentTurn = function currentTurn() {    
    return { player: this.getActivePlayer(), turn: this.turn, pile: this._gameSet, table: this._tableSet, ending: this.isGameFinished };
};
...
Game.prototype.nextTurn = function nextTurn() {
    if (this.isGameFinished === false) {
        this.turn++;
        if (this.turn >= this._playersOrder.length) {
            this.turn = 0;
        }
    }
    this.debug("Tour de jeux :" + JSON.stringify(this));
    return { player: this.getActivePlayer(), turn: this.turn , pile: this._gameSet, table: this._tableSet, ending: this.isGameFinished };
};
...
Game.prototype.foundPlayablesDominosOnActivePlayer = function foundPlayablesDominos() {    
    return Game.foundPlayablesDominos(this._players.item(this._playersOrder[this.turn]).hand, this._tableSet);
};
...
Game.prototype.playDomino = function playDomino(playerId, dominoIdx, sideIdx) {
    ...
    return { result:result, player: player, turn: this.turn , pile: this._gameSet, table: this._tableSet, ending: this.isGameFinished };
};
    ...
    if (!this.isGameFinished) {
        if (player) {
            if (this._gameSet.length > 0) {
                var newDomino = this._gameSet.pop();
...
Game.prototype.getWinner = function getWinner() {
    ...
    return { players: this._players, winner: winner, turn: this.turn, pile: this._gameSet, table: this._tableSet, ending: this.isGameFinished };
};
